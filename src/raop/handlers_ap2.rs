...
#[cfg(feature = "ap2")]
/// AP2 `/pair-add`: store a controller LTPK added by a paired admin.
pub(crate) fn handle_pair_add(
    conn: &mut RaopConnection,
    request: &HttpRequest,
    response: &mut HttpResponse,
) -> Option<Vec<u8>> {
    response.add_header("Content-Type", "application/octet-stream");
    if !conn.is_ap2 {
        tracing::warn!("pair-add rejected before AP2 encryption is active");
        return Some(pairing_homekit::pairing_error_response(2));
    }
    match request
        .data()
        .and_then(|data| pairing_homekit::process_pair_add(data).ok())
    {
        Some((client_id, client_pk)) => {
            conn.shared.pairing_store.put(&client_id, client_pk);
            tracing::info!(client_id, "AP2 pair-add stored client key");
            Some(pairing_homekit::pair_management_success_response())
        }
        None => {
            tracing::warn!("pair-add failed");
            Some(pairing_homekit::pairing_error_response(2))
        }
    }
}

#[cfg(feature = "ap2")]
/// AP2 `/pair-remove`: remove a controller LTPK.
pub(crate) fn handle_pair_remove(
    conn: &mut RaopConnection,
    request: &HttpRequest,
    response: &mut HttpResponse,
) -> Option<Vec<u8>> {
    response.add_header("Content-Type", "application/octet-stream");
    if !conn.is_ap2 {
        tracing::warn!("pair-remove rejected before AP2 encryption is active");
        return Some(pairing_homekit::pairing_error_response(2));
    }
    match request
        .data()
        .and_then(|data| pairing_homekit::process_pair_remove(data).ok())
    {
        Some(client_id) => {
            conn.shared.pairing_store.remove(&client_id);
            tracing::info!(client_id, "AP2 pair-remove removed client key");
            Some(pairing_homekit::pair_management_success_response())
        }
        None => {
            tracing::warn!("pair-remove failed");
            Some(pairing_homekit::pairing_error_response(2))
        }
    }
}
...
#[cfg(feature = "ap2")]
/// AP2 `/pair-list`: list stored controller LTPKs.
pub(crate) fn handle_pair_list(
    conn: &mut RaopConnection,
    request: &HttpRequest,
    response: &mut HttpResponse,
) -> Option<Vec<u8>> {
    response.add_header("Content-Type", "application/octet-stream");
    if !conn.is_ap2 {
        tracing::warn!("pair-list rejected before AP2 encryption is active");
        return Some(pairing_homekit::pairing_error_response(2));
    }
    match request
        .data()
        .and_then(|data| pairing_homekit::process_pair_list(data).ok())
    {
        Some(()) => Some(pairing_homekit::pair_list_response(conn.shared.pairing_store.list())),
        None => {
            tracing::warn!("pair-list failed");
            Some(pairing_homekit::pairing_error_response(2))
        }
    }
}
