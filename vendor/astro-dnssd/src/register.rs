...
pub struct DNSServiceBuilder {
    ...
    pub(crate) txt_ordered: Option<Vec<(String, String)>>,
}
impl DNSServiceBuilder {
    ...
    /// Includes a TXT record for the service (unordered)
    pub fn with_txt_record(mut self, txt: HashMap<String, String>) -> DNSServiceBuilder {
        self.txt = Some(txt);
        self
    }

    /// Includes a TXT record with guaranteed insertion order.
    pub fn with_txt_ordered(mut self, txt: Vec<(String, String)>) -> DNSServiceBuilder {
        self.txt_ordered = Some(txt);
        self
    }
